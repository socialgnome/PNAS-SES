...
#### FigS1A in the PNAS manuscript

#### Prerequisites: 1. OmnibusTest_FigS1A.R
...
library(stringr)
loadfonts()
res = readRDS(str_c(here("Res/ThirdSubmission/"), "Omnibus_FigS1A.rds"))
...
tiff(str_c(here("Res/ThirdSubmission/Figures/"), "FigS1A_C.tiff"), units="px", width=(6*600), height=(6*800), res=600)
...
tiff(str_c(here("Res/ThirdSubmission/Figures/"), "FigS1A_1.tiff"), units="px", width=(6*600), height=(6*720), res=600)
...
tiff(str_c(here("Res/ThirdSubmission/Figures/"), "FigS1A_2.tiff"), units="px", width=(6*600), height=(6*700), res=600)
p = ggplot(res, aes(x = Treatment,y  = Signature, color = Class, fill = Class,  size = pval2)) +
  geom_point(alpha  = 0.5,shape = 21, stroke = 1) +
