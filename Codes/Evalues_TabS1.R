...
e_fun = function(treatment, controls, mediator, subjectData, AID, expressionData) {
  ## Data
  subData = subjectData[AID,]
  data = data.frame(Y = expressionData, subData)
  ## fit.y
  mod_formula = as.formula(str_c("Y ~ ",str_c(c(treatment,controls), collapse = " + ")))
  fit_y = lm(mod_formula,data)
  ## Extract results #2 is always treatment
  est = summary(fit_y)$coefficients[2,1]
  se = summary(fit_y)$coefficients[2,2]
  sd = summary(fit_y)$sigma
  allres = c(est,se,sd)
  return(allres)
}
...
fullout_up = foreach(i=1:length(expr) , .options.snow = opts) %dopar% {  
  out = e_fun(treat_m[[i]], controls, mediator_m[[i]], subData, subjectdata[[i]], expr[[i]])
  out
}
close(pb)
...
### Save results
saveRDS(allresup, str_c(here("Res/ThirdSubmission/Clustering/"),"/Evalues_Up_With_1KI.rds"))
saveRDS(allresdown, str_c(here("Res/ThirdSubmission/Clustering/"),"/Evalues_Down_With_1KI.rds"))
...
### Save results
saveRDS(allresup, str_c(here("Res/ThirdSubmission/Clustering/"),"/Evalues_Up_Without_1KI.rds"))
saveRDS(allresdown, str_c(here("Res/ThirdSubmission/Clustering/"),"/Evalues_Down_Without_1KI.rds"))
